...
export const SEVERITY_ORDER: ScanIssue['severity'][] = ['critical', 'serious', 'moderate', 'minor'];
...
export function ScoreCircle({ score, size = 140, showLabel = true }: { score: number; size?: number; showLabel?: boolean }) {
  const color = scoreColor(score);
  const dashoffset = CIRCUMFERENCE * (1 - score / 100);
  const fontSize = (size / 140) * 28;
  const subFontSize = (size / 140) * 12;
  return (
    <div className="flex flex-col items-center">
      <svg width={size} height={size} viewBox="0 0 140 140">
        ...
        />
        <text x="70" y="65" textAnchor="middle" dominantBaseline="middle" fontSize={fontSize} fontWeight="700" fill={color}>
          {score}
        </text>
        <text x="70" y="86" textAnchor="middle" fontSize={subFontSize} fill="#94a3b8">
          / 100
        </text>
      </svg>
      {showLabel && (
...
export function LoadingView({ status, progress }: { status: ScanStatus; progress: number | null }) {
  const steps = [
...
export function ResultView({ report, cached }: { report: ReportData; cached: boolean }) {
